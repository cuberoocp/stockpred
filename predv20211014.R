...
feature = rbind(fread("data2/labelset2021_20201127.csv", colClasses = c(code = "character")))
...
# record = record[,tmp:=(score5*1.2-1.02*1.2-yper/2 - rid/1000/2 )]
# record = record[,tmp2:=ifelse(tmp>0.12,0.06
#                               ,ifelse(tmp>0.08,0.05
#                               ,ifelse(tmp>0.05,0.04
#                               ,ifelse(tmp>0.03,0.03
#                               ,ifelse(tmp>0.02,0.02
#                               ,ifelse(tmp>0.01,0.01
#                                       ,ifelse(tmp< -0.13,-0.08
#                                       ,ifelse(tmp< -0.11,-0.07
#                                       ,ifelse(tmp< -0.09,-0.06
#                                               ,ifelse(tmp< -0.07,-0.05
#                                       ,ifelse(tmp< -0.05,-0.04
#                                               ,ifelse(tmp< -0.03,-0.03
#                                                       ,ifelse(tmp< -0.02,-0.02
#                                                       ,ifelse(tmp< -0.01,-0.01,0)
#                   ))))))))))))),by=code]
# record = record[,tper:=ifelse(yper+tmp2>0,yper+tmp2,0)]
...
record2 = record2[, tmp := ((score5 - 1.02) * 1.4 - rid / 30000) * 0.99, by = code]
...
while (abs(b - 1) > 0.05 + 0.001 * times |
       sum(record2[, .(tprice = tcnt * close)]$tprice) / zijin / 10000 > 1) {
  xiuzheng = xiuzheng + ifelse(b > 1,-0.005, 0.006)
  record2 = record2[, tmp := (( score5 - 1.02 + xiuzheng ) * 1.4 - rid / 30000) * 0.99 , by = code]
...
record2 = record2[tcnt>0 | ycnt>0 ,][order(-score5)]
...
cookie = 'kbz_newcookie=1; kbzw_r_uname=maoxiong; kbzw__user_login=7Obd08_P1ebax9aX5MPl2eDm4d-Cq47k2ujc8NvpxtS-otvZ25bSldXb2Z2ynqnEqJLd26qvlaiS19uqobDQrMamgrKk6OHFzr6fqq6eqY2yj8ui1dSexdDqyuDl1piumqeCnrjg5dfn2OOBws2Vmqmap52WuODlqayckNmqrbCJ6-Kxm6mPp6CTv8bTzOOop5mqnKeTppKXvdzqxtbQ7KiunKec; kbzw__Session=s7pbknfrk2eqcd9p3j8i1kd533'
...
if (nrow(jsl1)>100) {
  ...
  tmp = jsl1[,.(zzcode=bond_id,zzname=bond_nm
                ...
                ,tflag =ifelse( abs(as.numeric(sub("%", "",sincrease_rt)))>4.5    
                                ,1,0)  
  )]
  ...
  zhuanzhai = merge(zhuanzhai,adjust2[,.(zzcode,power1,power1d,power2,power2d,power3,power3d)],by="zzcode",all.x = T)
  ...
  zhuanzhai = zhuanzhai[,.(zzcode,name,zzname,price,yijia,eb=as.integer(eb),remain,pingji=pingji/2,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday,tflag,ytmorder
                           ,score,cost1,cost2,cost3,cost4)]
  
  zhuanzhaitrain = fread("data2/zhuanzhaitrain.csv")
  
  newtrain = rbind(zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=-15,price=price*1.15,yijia=yijia*1.15+0.15,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=-10,price=price*1.10,yijia=yijia*1.1+0.10,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=-8,price=price*1.08,yijia=yijia*1.08+0.08,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=-6,price=price*1.06,yijia=yijia*1.06+0.06,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=-4,price=price*1.04,yijia=yijia*1.04+0.04,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)] , 
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=-2,price=price*1.02,yijia=yijia*1.02+0.02,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=0,price,yijia,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=2,price=price*0.98,yijia=yijia*0.98-0.02,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=4,price=price*0.96,yijia=yijia*0.96-0.04,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=6,price=price*0.94,yijia=yijia*0.94-0.06,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=8,price=price*0.92,yijia=yijia*0.92-0.08,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=10,price=price*0.9,yijia=yijia*0.9-0.1,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)],
                   zhuanzhai[!zzcode %in% tmp2$zzcode & tflag==0,.(label=15,price=price*0.85,yijia=yijia*0.85-0.15,eb,remain,pingji,pb,power1,power1d,power2,power2d,power3,power3d,lifecycle,rate1,rate2,redeemprice,rday)])
  
  newtrain = newtrain[,ds:=as.character(Sys.Date()) ]
  
  zhuanzhaitrain = zhuanzhaitrain[,.(label,price,yijia,eb,remain,pingji
                                     ,pb
                                     ,power1= ifelse(power>=0.1,1,0)
                                     ,power1d=NA
                                     ,power2=NA
                                     ,power2d=NA
                                     ,power3=NA
                                     ,power3d=NA
                                     ,lifecycle,rate1,rate2,redeemprice,rday,ds)
                                  ]
  library(catboost)
  train_pool = catboost.load_pool(as.matrix(apply(zhuanzhaitrain[,2:18,with=F],2,as.numeric)),
                             label = as.matrix(zhuanzhaitrain[,label]))
  test_pool = catboost.load_pool(as.matrix(apply(newtrain[,2:18,with=F],2,as.numeric)),
                                  label = as.matrix(newtrain[,label]))
  
  fit_params = list(iterations = 7000,
  ...
  cv_zz_model = catboost.train(learn_pool=train_pool,test_pool = test_pool
                          ,params=fit_params
                          )
  
  #[7000]	train-mae:1.173976	test-mae:1.669062 
  save(cv_zz_model, file="model/cv_zz_model.Rdata")
  

  } else   {stop("cookie error")}
